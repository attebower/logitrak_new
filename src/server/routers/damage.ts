...
export const damageRouter = router({
  damageReport: router({
    ...
    listActive: workspaceProcedure
      ...
      .query(async ({ ctx }) => {
        return ctx.prisma.damageReport.findMany({
          ...
          orderBy: { reportedAt: "desc" },
          include: {
            equipment: { select: { id: true, serial: true, name: true, status: true, damageStatus: true, category: { select: { name: true, groupName: true } } } },
            reporter: { select: { id: true, displayName: true, email: true } },
            repairLogs: {
              orderBy: { repairedAt: "desc" },
              take: 1,
              include: { repairer: { select: { id: true, displayName: true, email: true } } },
            },
          },
        });
      }),
    ...
    listAll: workspaceProcedure
      ...
      .query(async ({ ctx, input }) => {
        const damageStatusFilter =
          input.resolved === true
            ? { damageStatus: { in: ["repaired" as const] } }
            : input.resolved === false
            ? { damageStatus: { in: ["damaged" as const, "under_repair" as const] } }
            : undefined;

        return ctx.prisma.damageReport.findMany({
          where: {
            workspaceId: ctx.workspaceId!,
            ...(damageStatusFilter ? { equipment: damageStatusFilter } : {}),
          },
          orderBy: { reportedAt: "desc" },
          include: {
            equipment: { select: { id: true, serial: true, name: true, status: true, damageStatus: true, category: { select: { name: true, groupName: true } } } },
